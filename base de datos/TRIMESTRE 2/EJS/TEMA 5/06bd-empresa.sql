...
DELIMITER $$
CREATE PROCEDURE ac12mediaSalarial(IN CodDep CHAR(5), OUT salarioMedio DECIMAL)
BEGIN
SELECT AVG(SalEmp) INTO salarioMedio FROM empleado e WHERE e.CodDep=codDep;
END $$
DELIMITER ;

DELIMITER $$
CREATE FUNCTION ac12categoriaDepartamento(CodDep CHAR(5)) RETURNS VARCHAR(50)
BEGIN

DECLARE salarioMedio DECIMAL;
DECLARE categoria VARCHAR(50);

call ac12mediaSalarial(CodDep, salarioMedio);

IF salarioMedio < 2000000 THEN
    SET categoria = 'Bajo';
ELSEIF salarioMedio <= 500000 THEN
    SET categoria = 'Medio';
ELSE
    SET categoria= 'Alto';
...
DELIMITER $$
CREATE OR REPLACE PROCEDURE ac12actualizaInforme(IN CodDep CHAR(5))
BEGIN
DECLARE salarioMedio DECIMAL;
DECLARE categoria VARCHAR(10);
DECLARE numEmpleados INT;
DECLARE nomDep VARCHAR(40);
SELECT NomDep, COUNT(CodEmp) INTO nomDep, numEmpleados FROM departamento d LEFT JOIN empleado e ON d.CodDep = e.CodDep WHERE d.CodDep = CodDep;
call ac12mediaSalarial(CodDep, salarioMedio);
SET categoria = ac12categoriaDepartamento(CodDep);
INSERT INTO informe_salarial (CodDep, NomDep, NumEmpleados, SalarioMedio, Categoria) VALUES (CodDep, nomDep, numEmpleados, salarioMedio, categoria);

END $$
DELIMITER ;

CALL ac12actualizaInforme('ADMZS');
CALL ac12actualizaInforme('DIRGE');
CALL ac12actualizaInforme('IN&DI');
CALL ac12actualizaInforme('JEFZS');   